...
public class DomainSlot {
	...
	/**
	 * The handler for all attribute and index conditions. If a parameter
	 * constraints fixes the value of a parameter, the handler checks all
	 * conditions.
	 */
	...
	/**
	 * If the node is a parameter of the rule, holds the parameter name.
	 */
	final String nodeNameParameter;

	/**
	 * Constructor.
	 * 
	 * @param conditionHandler
	 *            Condition handler to be used.
	 * @param usedObjects
	 *            Used objects.
	 * @param nodeNameParameter
	 * @param options
	 *            Options.
	 */
	public DomainSlot(ConditionHandler conditionHandler,
			Set<EObject> usedObjects, boolean injective, boolean dangling,
			boolean deterministic, boolean inverseMatchingOrder,
			String nodeNameParameter) {
		...
		this.nodeNameParameter = nodeNameParameter;
	}
	 ...
	 *            Variable to be set.
	 * @param domainMap
	 *            The domain map to be used.
	 * @param graph
	 *            The target graph.
	 * @return <code>true</code> if the instantiation was successful.
	 */
	public boolean instantiate(Variable variable,
		...
		// Set the node parameter?
		if (nodeNameParameter != null) {
			conditionHandler.setParameter(nodeNameParameter, value);
		}
		...
		if (!checkedVariables.contains(variable)) {
			...
			for (AttributeConstraint constraint : variable.attributeConstraints) {
				...
				UnaryConstraint unaryUserConstraint = variable.attributeUserConstraints
						.get(constraint);
			...
			for (ContainmentConstraint constraint : variable.containmentConstraints) {
				DomainSlot targetSlot = domainMap
						.get(constraint.targetVariable);
			...
			for (ReferenceConstraint constraint : variable.referenceConstraints) {
				...
				BinaryConstraint binaryUserConstraint = variable.binaryUserConstraints
						.get(constraint);
	...
	public boolean unlock(Variable sender) {
		...
		for (int i = refCount + conCount - 1; i >= 0; i--) {
			BinaryConstraint constraint = (i >= refCount) ? sender.containmentConstraints
					.get(i - refCount) : sender.referenceConstraints.get(i);
		...
		//
		if (nodeNameParameter != null)
			conditionHandler.unsetParameter(nodeNameParameter);
	...
	/**
	 * Clears this domain slot to the state before it was initialized. Only the
	 * variable that originally initialized this domain slot is able to clear
	 * it.
	 * 
	 * @param sender
	 *            The variable which uses this domain slot.
	 */
	...
	/**
	 * Resets this domain slot to the state before it was initialized. Only the
	 * variable that originally initialized this domain slot is able to reset
	 * it. Resetting means clearing and additionally resetting the temporary
	 * domain.
	 * 
	 * @param sender
	 *            The variable which uses this domain slot.
	...
	/**
	 * Re-check the constraint of a variable. It is assumed that this slot is
	 * initialized and locked.
	 * 
	 * @param variable
	 *            Variable to be re-checked.
	 * @param domainMap
	 *            The domain map.
	 * @return <code>true</code> if all constraint were successfully checked.
	 */
	public boolean recheck(Variable variable,
	...
	/**
	 * @param temporaryDomain
	 *            the temporaryDomain to set
	 */
	...
	public Map<String, Object> getParameterValues() {
		return conditionHandler.getParameterValues();
	}
